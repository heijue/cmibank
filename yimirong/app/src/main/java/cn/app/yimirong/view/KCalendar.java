...
public class KCalendar extends ViewFlipper  {
    ...
    public static final int COLOR_TX_WEEK_TITLE = Color.parseColor("#000000"); // 星期标题文字颜色
...
//        gd = new GestureDetector(this);
        // 初始化日历翻动动画
        push_left_in = AnimationUtils.loadAnimation(getContext(), R.anim.push_left_in_ca);
        push_left_out = AnimationUtils.loadAnimation(getContext(), R.anim.push_left_out_ca);
    ...
    private void drawFrame(LinearLayout oneCalendar) {
        ...
        LinearLayout.LayoutParams layout = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, 60);
        Resources res = getResources();
        tb = res.getDimension(R.dimen.sign_dimen);
        layout.setMargins(0, (int) (tb * 1.2), 0, 0);
...
//                    }
    ...
    private void setMarker(RelativeLayout group, int i, int j) {
        ...
        if (marksMap.get(dates[i][j]) != null) {
            if (childCount < 2) {
                RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams((int) (20 * 2.0), (int) (20 * 2.0));
                ...
                //                markView.setBackgroundResource(R.drawable.calendar_bg_tag);
//                markView.setBackgroundResource(R.drawable.ic_jifen_gou);
