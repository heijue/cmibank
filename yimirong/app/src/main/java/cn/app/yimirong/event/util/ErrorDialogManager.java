...
/**
 * Central class for cn.app that want to use event based error dialogs.<br/>
 * <br/>
 * How to use:
 * <ol>
 * <li>Set the {@link #factory} to configure dialogs for your cn.app, typically in
...
public class ErrorDialogManager {
	...
	public static class SupportManagerFragment extends Fragment {
		...
		public void onEventMainThread(ThrowableFailureEvent event) {
			...
			DialogFragment existingFragment = (DialogFragment) fm
			...
			DialogFragment errorFragment = (DialogFragment) factory
					.prepareErrorFragment(event, finishAfterDialog,
		...
		public static void attachTo(Activity activity, Object executionScope,
									boolean finishAfterDialog, Bundle argumentsForErrorDialog) {
			FragmentManager fm = ((FragmentActivity) activity)
					.getSupportFragmentManager();
			SupportManagerFragment fragment = (SupportManagerFragment) fm
					.findFragmentByTag(TAG_ERROR_DIALOG_MANAGER);
			if (fragment == null) {
				fragment = new SupportManagerFragment();
				fm.beginTransaction().add(fragment, TAG_ERROR_DIALOG_MANAGER)
						.commit();
				fm.executePendingTransactions();
			}
	...
	public static class HoneycombManagerFragment extends android.app.Fragment {
		...
		public void onEventMainThread(ThrowableFailureEvent event) {
			...
			android.app.DialogFragment errorFragment = (android.app.DialogFragment) factory
					.prepareErrorFragment(event, finishAfterDialog,
							argumentsForErrorDialog);
			if (errorFragment != null) {
				errorFragment.show(fm, TAG_ERROR_DIALOG);
	...
	public static void attachTo(Activity activity, Object executionScope,
		...
		if (factory == null) {
			throw new RuntimeException(
					"You must set the static factory field to configure error dialogs for your cn.app.");
		}
		if (isSupportActivity(activity)) {
			SupportManagerFragment.attachTo(activity, executionScope,
					finishAfterDialog, argumentsForErrorDialog);
		} else {
			HoneycombManagerFragment.attachTo(activity, executionScope,
					finishAfterDialog, argumentsForErrorDialog);
		}
	}
	...
	private static boolean isSupportActivity(Activity activity) {
		...
		for (Class<?> c = activity.getClass().getSuperclass(); ; c = c
				.getSuperclass()) {
			if (c == null) {
				throw new RuntimeException("Illegal activity type: "
						+ activity.getClass());
			}
			String name = c.getName();
			if (name.equals("android.support.v4.cn.app.FragmentActivity")) {
				isSupport = true;
				break;
			} else if (name.startsWith("com.actionbarsherlock.cn.app")
					&& (name.endsWith(".SherlockActivity")
					|| name.endsWith(".SherlockListActivity") || name
					.endsWith(".SherlockPreferenceActivity"))) {
				throw new RuntimeException(
						"Please use SherlockFragmentActivity. Illegal activity: "
								+ name);
			} else if (name.equals("android.cn.app.Activity")) {
				if (Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB) {
					throw new RuntimeException(
							"Illegal activity without fragment support. Either use Android 3.0+ or android.support.v4.cn.app.FragmentActivity.");
				}
				break;
			}
		}
	...
	protected static void checkLogException(ThrowableFailureEvent event) {
		if (factory.config.logExceptions) {
			...
			Log.i(tag, "Error dialog manager received exception",
					event.throwable);
		}
	}

	private static boolean isInExecutionScope(Object executionScope,
											  ThrowableFailureEvent event) {
		if (event != null) {
			Object eventExecutionScope = event.getExecutionScope();
			if (eventExecutionScope != null
					&& !eventExecutionScope.equals(executionScope)) {
