...
public abstract class ErrorDialogFragmentFactory<T> {
	 ...
	 */
	protected T prepareErrorFragment(ThrowableFailureEvent event,
		...
		if (!bundle.containsKey(ErrorDialogManager.KEY_FINISH_AFTER_DIALOG)) {
			bundle.putBoolean(ErrorDialogManager.KEY_FINISH_AFTER_DIALOG,
					finishAfterDialog);
		}
		...
		if (!bundle.containsKey(ErrorDialogManager.KEY_ICON_ID)
				&& config.defaultDialogIconId != 0) {
			bundle.putInt(ErrorDialogManager.KEY_ICON_ID,
					config.defaultDialogIconId);
		}
	 ...
	 */
	protected abstract T createErrorFragment(ThrowableFailureEvent event,
											 Bundle arguments);
	...
	public static class Support extends ErrorDialogFragmentFactory<Fragment> {
		...
		protected Fragment createErrorFragment(ThrowableFailureEvent event,
											   Bundle arguments) {
	...
	public static class Honeycomb extends
			ErrorDialogFragmentFactory<android.app.Fragment> {

		public Honeycomb(ErrorDialogConfig config) {
			super(config);
		}

		protected android.app.Fragment createErrorFragment(
				ThrowableFailureEvent event, Bundle arguments) {
