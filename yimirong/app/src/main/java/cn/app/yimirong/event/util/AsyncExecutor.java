...
public class AsyncExecutor {
	...
	private AsyncExecutor(Executor threadPool, EventBus eventBus,
						  Class<?> failureEventType, Object scope) {
		this.threadPool = threadPool;
		this.eventBus = eventBus;
		this.scope = scope;
		try {
			failureEventConstructor = failureEventType
			...
			public void run() {
				try {
					...
					try {
						...
						throw new RuntimeException(
								"Could not create failure event", e1);
					}
