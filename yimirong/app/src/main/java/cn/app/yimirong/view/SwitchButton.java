...
public class SwitchButton extends CompoundButton {
	...
	private void init(AttributeSet attrs) {
		...
		switchScroller = new SwitchScroller(getContext(),
		...
		if (attrs != null && getContext() != null) {
			...
			if (typedArray != null) {
				withTextInterval = (int) typedArray.getDimension(
						R.styleable.SwitchButton_withTextInterval, 0.0f);
				setDrawables(
						typedArray
								.getDrawable(R.styleable.SwitchButton_frameDrawable),
						typedArray
								.getDrawable(R.styleable.SwitchButton_stateDrawable),
						typedArray
								.getDrawable(R.styleable.SwitchButton_stateMaskDrawable),
						typedArray
								.getDrawable(R.styleable.SwitchButton_sliderDrawable));
				typedArray.recycle();
			}
		}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		switch (MeasureSpec.getMode(heightMeasureSpec)) {
			case MeasureSpec.AT_MOST:// 如果heightSize是当前视图可使用的最大宽度
				measureHeight = (frameDrawable != null ? frameDrawable
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		if (drawables != null) {
			if (drawables.length > 1 && drawables[1] != null) {
				drawableTopHeight = drawables[1].getIntrinsicHeight()
			...
			if (drawables.length > 3 && drawables[3] != null) {
				drawableBottomHeight = drawables[3].getIntrinsicHeight()
						+ getCompoundDrawablePadding();
			}
		}

		buttonLeft = (getWidth()
				- (frameDrawable != null ? frameDrawable.getIntrinsicWidth()
				: 0) - getPaddingRight() - drawableRightWidth);
		buttonTop = (getHeight()
				- (frameDrawable != null ? frameDrawable.getIntrinsicHeight()
				: 0) + drawableTopHeight - drawableBottomHeight) / 2;
		buttonRectF.set(buttonLeft, buttonTop, buttonLeft
				+ (frameDrawable != null ? frameDrawable.getIntrinsicWidth()
				: 0), buttonTop
				+ (frameDrawable != null ? frameDrawable.getIntrinsicHeight()
				: 0));
	...
	protected void onDraw(Canvas canvas) {
		...
		if (stateDrawable != null && stateMaskDrawable != null) {
			...
			if (stateMaskDrawable != null && stateBitmap != null
					&& !stateBitmap.isRecycled()) {
				// 保存并创建一个新的透明层，如果不这样做的话，画出来的背景会是黑的
				int src = canvas.saveLayer(0, 0, getWidth(), getHeight(),
						paint, Canvas.MATRIX_SAVE_FLAG | Canvas.CLIP_SAVE_FLAG
	...
	protected boolean verifyDrawable(Drawable who) {
		return super.verifyDrawable(who) || who == frameDrawable
				|| who == stateDrawable || who == stateMaskDrawable
				|| who == sliderDrawable;
	}
	...
	public void jumpDrawablesToCurrentState() {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
			...
			if (frameDrawable != null)
				frameDrawable.jumpToCurrentState();
			if (stateDrawable != null)
				stateDrawable.jumpToCurrentState();
			if (stateMaskDrawable != null)
	...
	public void setDrawables(Drawable frameBitmap, Drawable stateDrawable,
		...
		if (frameBitmap == null || stateDrawable == null
				|| stateMaskDrawable == null || sliderDrawable == null) {
			if (!isInEditMode()) {
		...
		if (!isInEditMode()) {
			this.frameDrawable.setBounds(0, 0,
					this.frameDrawable.getIntrinsicWidth(),
					this.frameDrawable.getIntrinsicHeight());
			this.frameDrawable.setCallback(this);
			this.stateDrawable.setBounds(0, 0,
					this.stateDrawable.getIntrinsicWidth(),
					this.stateDrawable.getIntrinsicHeight());
			this.stateDrawable.setCallback(this);
			this.stateMaskDrawable.setBounds(0, 0,
					this.stateMaskDrawable.getIntrinsicWidth(),
					this.stateMaskDrawable.getIntrinsicHeight());
			this.stateMaskDrawable.setCallback(this);
			this.sliderDrawable.setBounds(0, 0,
					this.sliderDrawable.getIntrinsicWidth(),
	...
	public void setDrawableResIds(int frameDrawableResId,
								  int stateDrawableResId, int stateMaskDrawableResId,
								  int sliderDrawableResId) {
		if (getResources() != null) {
			setDrawables(getResources().getDrawable(frameDrawableResId),
	...
	private int setSlideX(int newSlideX) {
		// 防止滑动超出范围
		if (newSlideX < tempMinSlideX)
			newSlideX = tempMinSlideX;
		if (newSlideX > tempMaxSlideX)
	...
	private class SwitchScroller implements Runnable {
		...
		public SwitchScroller(Context context,
		...
		public void startScroll(boolean checked) {
			scroller.startScroll(tempSlideX, 0, (checked ? tempMinSlideX
					: tempMaxSlideX) - tempSlideX, 0, duration);
			post(this);
		}
